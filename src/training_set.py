...
class TrainingSet(torch.utils.data.Dataset):
    def __init__(self, restore, bw=100, n_features = 2):
        self.ds = None
        self.bw = bw
    ...
    def generateAll(self, datasource):
        ...
        self.anchor_features = [np.zeros((self.n_features, double_bw, double_bw))] * n_clouds
        self.positive_features = [np.zeros((self.n_features, double_bw, double_bw))] * n_clouds
        self.negative_features = [np.zeros((self.n_features, double_bw, double_bw))] * n_clouds
        ...
        for i in range(self.ds.start_cached, self.ds.end_cached):
            self.anchor_features[i], self.positive_features[i], self.negative_features[i] = self.createFeature(
                a_pcl_features[i], p_pcl_features[i], n_pcl_features[i])
    ...
    def createFeature(self, a_cloud, p_cloud, n_cloud):
        double_bw = 2 * self.bw
        anchor_features = np.zeros((self.n_features, double_bw, double_bw))
        positive_features = np.zeros((self.n_features, double_bw, double_bw))
        negative_features = np.zeros((self.n_features, double_bw, double_bw))
