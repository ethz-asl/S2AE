...
print(
    f"Shape of clouds is {cloud_features.shape} and sem clouds is {sem_cloud_features.shape}")
    ...
    with torch.no_grad():
        for batch_idx, (cloud, lidarseg_gt) in enumerate(data_loader):
            ...
            for i in range(0, n_batch):
                all_decoded_clouds[k] = pred_segmentation.cpu().data.numpy()[
                    i, :, :]
        ...
        print(
            f'[Testing] Average Pixel Accuracy per Class: {avg_pixel_acc_per_class.avg}')
