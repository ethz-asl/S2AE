...
print(
    f"Shape clouds is {cloud_features.shape} and sem clouds is {sem_cloud_features.shape}")
...
for sample in val_samples:
    ...
    print(
        f"Shape clouds is {val_features.shape} and sem clouds is {sem_val_features.shape}")
...
assert n_val_sets > 0
print(
    f'Loaded {n_val_sets} validation sets. Setting up the validation loaders now.')
print('\n')

train_set = TrainingSetLidarSeg(cloud_features, sem_cloud_features)
split = ExternalSplitter(train_set, val_sets[0])
train_loader, val_loader = split.get_split(
...
def validate_lidarseg(net, criterion, optimizer, writer, epoch, n_iter):
    ...
    with torch.no_grad():
        for val_loader in val_loaders:
            ...
            for batch_idx, (cloud, lidarseg_gt) in enumerate(val_loader):
                ...
                writer.add_scalar('Validation/Loss',
                                  loss.mean().item(), n_iter)

                pred_segmentation = torch.argmax(enc_dec_cloud, dim=1)
                mask = lidarseg_gt <= 0
                pred_segmentation[mask] = 0

                pixel_acc, pixel_acc_per_class, jacc, dice = eval_metrics(
                    lidarseg_gt, pred_segmentation, num_classes=n_classes)
            ...
            print(
                '============================================================================')
            print(
                f'Validation Results for Validation Loader: {val_samples[k]}.')

            writer.add_scalar(
                f'Validation/{val_samples[k]}/AvgPixelAccuracy', avg_pixel_acc.avg, epoch_p_1)
            writer.add_scalar(
                f'Validation/{val_samples[k]}/AvgPixelAccuracyPerClass', avg_pixel_acc_per_class.avg, epoch_p_1)
            writer.add_scalar(
                f'Validation/{val_samples[k]}/AvgJaccardIndex', avg_jacc.avg, epoch_p_1)
            writer.add_scalar(
                f'Validation/{val_samples[k]}/AvgDiceCoefficient', avg_dice.avg, epoch_p_1)

            print('\n')
            print(
                f'[Validation for epoch {epoch_p_1}] Average Pixel Accuracy: {avg_pixel_acc.avg}')
            print(
                f'[Validation for epoch {epoch_p_1}] Average Pixel Accuracy per Class: {avg_pixel_acc_per_class.avg}')
            print(
                f'[Validation for epoch {epoch_p_1}] Average Jaccard Index: {avg_jacc.avg}')
            print(
        ...
        writer.add_scalar(f'Validation/AvgPixelAccuracy',
                          gl_avg_pixel_acc.avg, epoch_p_1)
        writer.add_scalar(f'Validation/AvgPixelAccuracyPerClass',
                          gl_avg_pixel_acc_per_class.avg, epoch_p_1)
        writer.add_scalar(f'Validation/AvgJaccardIndex',
                          gl_avg_jacc.avg, epoch_p_1)
        writer.add_scalar(f'Validation/AvgDiceCoefficient',
                          gl_avg_dice.avg, epoch_p_1)

        print('\n')
        print(
            f'[Validation for epoch {epoch_p_1}] Average Pixel Accuracy: {gl_avg_pixel_acc.avg}')
        print(
            f'[Validation for epoch {epoch_p_1}] Average Pixel Accuracy per Class: {gl_avg_pixel_acc_per_class.avg}')
        print(
            f'[Validation for epoch {epoch_p_1}] Average Jaccard Index: {gl_avg_jacc.avg}')
        print(
...
def test_lidarseg(net, criterion, writer):
    ...
    with torch.no_grad():
        for batch_idx, (cloud, lidarseg_gt) in enumerate(test_loader):
            ...
            writer.add_scalar('Test/PixelAccuracyPerClass',
                              pixel_acc_per_class, n_iter)
            ...
            for i in range(0, n_batch):
                ...
                all_decoded_clouds[k] = enc_dec_cloud.cpu().data.numpy()[
                    i, :, :, :]
        ...
        writer.add_scalar('Test/AvgPixelAccuracyPerClass',
                          avg_pixel_acc_per_class.avg, n_iter)
        ...
        print(
            f'Average Pixel Accuracy per Class: {avg_pixel_acc_per_class.avg}')
...
for epoch in tqdm(range(n_epochs)):
    ...
    train_iter = train_lidarseg(
        net, criterion, optimizer, writer, epoch, train_iter, loss_, t0)
    scheduler.step()

    val_iter = validate_lidarseg(
        net, criterion, optimizer, writer, epoch, val_iter)
...
if test_size > 0:
    ...
    input_clouds, decoded_clouds, gt_clouds = test_lidarseg(
        net, criterion, writer)
